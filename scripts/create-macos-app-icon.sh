...
OUTPUT_ICNS="$ROOT_DIR/src/GlucoDesk.Desktop/Assets/AppIcon/glucodesk-app-icon.icns"
PREVIEW_ICON="$ROOT_DIR/artifacts/icon-generation/macos-app-icon-preview.png"
ICONSET_DIR="$ROOT_DIR/artifacts/icon-generation/GlucoDesk.iconset"
TOOLS_DIR="$ROOT_DIR/artifacts/icon-generation/macos-app-icon-tools"
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  fail "macOS app icon generation must be run on macOS"
fi
...
info "generating optimized macOS app icon"

rm -rf "$ICONSET_DIR"
mkdir -p "$ICONSET_DIR"

"$PYTHON_BIN" - "$SOURCE_ICON" "$ICONSET_DIR" "$PREVIEW_ICON" <<'PY'
...
for file_name, size in icon_sizes:
    ...
    output.save(iconset_dir / file_name, format="PNG")
...
info "creating macOS .icns"
rm -f "$OUTPUT_ICNS"
iconutil -c icns "$ICONSET_DIR" -o "$OUTPUT_ICNS"

info "macOS app icon generated successfully"
echo "Icon:"
echo "  $OUTPUT_ICNS"
echo "Preview:"
echo "  $PREVIEW_ICON"
